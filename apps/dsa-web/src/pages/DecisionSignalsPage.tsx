...
import type {
  ...
  StockContext,
  TimelineFilters,
  TimelineMarketSource,
  TimelineRange,
  TimelineStatusFilter,
} from './decisionSignals/listModel';
import {
  ACTION_OPTIONS,
...
const DecisionSignalsPage: React.FC = () => {
  ...
  // 热门候选优先走后端 /stocks/search 热门模式（避免全量下载索引）；
  // 远程失败时降级为懒加载全量索引。
  const { index: fallbackStockIndex, load: loadFallbackStockIndex } = useStockIndex(true, { lazy: true });
  const [remotePopularIndex, setRemotePopularIndex] = useState<StockIndexItem[] | null>(null);
  useEffect(() => {
    let mounted = true;
    stocksApi
      .search('', 50)
      .then((response) => {
        if (mounted) {
          setRemotePopularIndex(toIndexItemsFromSearch(response.items ?? []));
        }
      })
      .catch(() => {
        if (mounted) {
          loadFallbackStockIndex?.();
        }
      });
    return () => {
      mounted = false;
  ...
  const popularCandidates = useMemo(
    () => toPopularCandidates(remotePopularIndex ?? fallbackStockIndex, STOCK_CANDIDATE_LIMIT),
    [remotePopularIndex, fallbackStockIndex],
  );
